...
export default function LiveMode() {
  ...
  // Fetch current mode and system status on mount
  useEffect(() => {
    ...
    const fetchSystemStatus = async () => {
      try {
        ...
        setMqttConnected(response.data.mqtt || false);
      } catch (error) {
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        >
          <div className={`bg-white/10 backdrop-blur-lg rounded-xl p-4 border ${isConnected ? 'border-[#2ecc71]' : 'border-[#e74c3c]'}`}>
            <div className="text-xs text-gray-300 mb-1">WebSocket</div>
          ...
          <div className={`bg-white/10 backdrop-blur-lg rounded-xl p-4 border ${mqttConnected ? 'border-[#2ecc71]' : 'border-[#f39c12]'}`}>
            <div className="text-xs text-gray-300 mb-1">MQTT</div>
            <div className={`text-lg font-bold flex items-center gap-2 ${mqttConnected ? 'text-[#2ecc71]' : 'text-[#f39c12]'}`}>
              <span className={`w-2 h-2 rounded-full ${mqttConnected ? 'bg-[#2ecc71]' : 'bg-[#f39c12]'} ${mqttConnected ? 'animate-pulse' : ''}`}></span>
              {mqttConnected ? 'Connected' : 'Disabled'}
            </div>
          </div>
