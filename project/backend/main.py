    ...
    try:
        ...
        if mqtt_client.is_connected():
            ...
            def on_nearby_hazard(topic, data):
                print(f"🚨 MQTT Alert: {data.get('hazard_type')} at {data.get('location', {})}")
            mqtt_client.subscribe_to_nearby_hazards(
                location={"lat": 0, "lng": 0},  # Will be updated by actual location
                radius_km=1.0,
                callback=on_nearby_hazard
            )
        else:
            print("⚠️  Warning: MQTT is not connected. MQTT features will be disabled.")
            print("   To enable MQTT, start MQTT broker or configure connection in .env")
    except Exception as e:
        print(f"⚠️  Warning: Could not initialize MQTT: {e}")
